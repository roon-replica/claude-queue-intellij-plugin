...
class TaskQueueServiceTest {
    ...
    @Test
    fun `레인 없는 작업은 세션을 공유하지 않는다`() {
        val a = enqueueAndRun("A", "/tmp")
        fake.complete()
        val b = enqueueAndRun("B", "/tmp")
        // 러너가 각자 새 세션을 만든다 (서비스는 지정하지 않음)
    ...
    @Test
    fun `같은 레인 작업은 한 세션을 이어 쓴다`() {
        val a = queue.addTodo("A", "/tmp", "리팩터링")
        val b = queue.addTodo("B", "/tmp", "리팩터링")
        queue.promote(a.id)
        fake.complete()
        queue.promote(b.id)

        assertEquals(a.sessionId, b.sessionId)
        assertEquals(a.sessionId, queue.laneSessionId("/tmp", "리팩터링"))
    }

    @Test
    fun `다른 레인은 다른 세션`() {
        val a = queue.addTodo("A", "/tmp", "레인1")
        val b = queue.addTodo("B", "/tmp", "레인2")
        queue.promote(a.id)
        fake.complete()
        queue.promote(b.id)
    ...
    @Test
    fun `같은 레인이라도 cwd 가 다르면 세션이 다르다`() {
        val a = queue.addTodo("A", "/tmp/x", "공용")
        val b = queue.addTodo("B", "/tmp/y", "공용")
        queue.promote(a.id)
        fake.complete()
        queue.promote(b.id)
        assertTrue(a.sessionId != b.sessionId)
    ...
    @Test
    fun `레인 초기화하면 다음 실행은 새 세션`() {
        val a = queue.addTodo("A", "/tmp", "레인")
        queue.promote(a.id)
        fake.complete()
        val first = a.sessionId

        queue.resetLane("/tmp", "레인")
        assertNull(queue.laneSessionId("/tmp", "레인"))

        val b = queue.addTodo("B", "/tmp", "레인")
        queue.promote(b.id)
        assertTrue(first != b.sessionId)
    }

    @Test
    fun `레인 목록은 cwd 별로 조회된다`() {
        queue.pause()
        val a = queue.addTodo("A", "/tmp/x", "레인1")
        val b = queue.addTodo("B", "/tmp/y", "레인2")
        queue.promote(a.id)
        queue.promote(b.id)
        assertEquals(listOf("레인1"), queue.lanes("/tmp/x"))
        assertEquals(listOf("레인2"), queue.lanes("/tmp/y"))
    }
